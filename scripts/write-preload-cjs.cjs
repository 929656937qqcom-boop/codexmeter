...
contextBridge.exposeInMainWorld('codexMeter', {
  ...
  redeemCloudPairingCode: (code) => ipcRenderer.invoke('cloud:redeemPairingCode', code),
  saveCloudSync: (enabled, endpoint, syncKey) => ipcRenderer.invoke('cloud:save', enabled, endpoint, syncKey),
  syncCloudNow: () => ipcRenderer.invoke('cloud:syncNow'),
  openCloudDashboard: () => ipcRenderer.invoke('cloud:openDashboard'),
