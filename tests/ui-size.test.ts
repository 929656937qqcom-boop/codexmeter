...
const quotaProviderSource = readFileSync('src/main/quotaProvider.ts', 'utf8')
...
describe('desktop sizing', () => {
  ...
  it('renders the main view as a dark reference-style quota dashboard', () => {
    ...
    expect(mainSource).toContain("ipcMain.handle('quota:refresh'")
    expect(appSource).toContain('checkAppUpdate')
    expect(appSource).toContain('saveDiagnosticsEnabled')
    expect(mainSource).toContain('promptForDownloadedUpdate(state)')
    expect(mainSource).toContain("buttons: ['重启并安装', '稍后']")
    expect(mainSource).toContain('installDownloadedUpdate()')
    ...
    expect(appSource).toContain(":class=\"{ 'is-loading': loading || usageLoading }\"")
    expect(appSource).toContain('refreshStatusLabel')
    expect(stylesSource).toContain('dashboard-refresh-spin')
    expect(mainSource).toContain('await syncCloudUsage(summary)')
  ...
  it('starts the widget as a compact floating ball', () => {
    ...
    expect(mainSource).toContain('const window = await createWidgetWindow()')
    ...
    expect(appSource).not.toContain('widget-orb-meter')
    expect(appSource).not.toContain('widget-orb-gauge')
    expect(appSource).toContain('widget-orb-value')
    expect(appSource).toContain('widget-orb-label')
    expect(appSource).not.toContain('widgetOrbStyle')
    expect(appSource).toContain('widgetWeeklyRemaining')
    expect(appSource).toContain('`is-${sevenDayState.value}`')
    expect(appSource).toContain('<span class="widget-orb-label">7D</span>')
    ...
    expect(stylesSource).not.toContain('.widget-orb::before')
    ...
    expect(stylesSource).not.toContain('--widget-week-ring')
    expect(stylesSource).toContain('#00bfa5')
    expect(selectorBlock(stylesSource, '.widget-orb')).not.toContain('backdrop-filter')
    expect(stylesSource).not.toContain('widget-water-sway')
    ...
    expect(mainSource).toContain('refreshQuotaInBackground')
    expect(mainSource).toContain('configureQuotaRefreshSchedule()')
    expect(mainSource).toContain("powerMonitor.on('resume'")
    expect(mainSource).toContain("powerMonitor.on('unlock-screen'")
    expect(mainSource).toContain('quotaRefreshInFlight')
    expect(mainSource).toContain("getSettings().updateChannel !== 'beta'")
    expect(quotaProviderSource.match(/AbortSignal\.timeout\(requestTimeoutMs\)/g)).toHaveLength(2)
    expect(appSource).not.toContain('configureAutoRefresh')
